...
const ladiesMedia = allMedia.filter((item) => ladiesHints.some((hint) => item.name.includes(hint)));
const leadVideo = ladiesMedia.find((item) => item.src.endsWith(".mp4"));
const galleryMedia = ladiesMedia.filter((item) => item.src !== leadVideo?.src);
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-32 pb-16 px-6 lg:px-12">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="font-display text-5xl md:text-7xl text-foreground leading-[0.95]">Ladies</h1>
        </div>
      </section>
            ...
            {galleryMedia.map((item) => (
              <div key={item.src} className="mb-4 break-inside-avoid overflow-hidden rounded-sm group">
                {item.src.endsWith(".mp4") ? (
                  ...
                  <img
                    src={item.src}
                    alt="Kai's Divo ladieswear look"
                    className="w-full h-auto object-cover transition-transform duration-700 group-hover:scale-105"
                    loading="lazy"
                  />
                )}
